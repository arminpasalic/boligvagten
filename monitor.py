#!/usr/bin/env python3
"""Boligvagten — watches Danish rental sites and alerts you the minute something new appears.
...
run). Sources live in sources/, one module per site — see CONTRIBUTING.md
...
STATE_FILE = ROOT / "seen_listings.json"
CONFIG_FILE = ROOT / "config.py"
EXAMPLE_FILE = ROOT / "config.example.py"
...
def ensure_config():
    ...
    print("Welcome! Created config.py from config.example.py — edit it to taste.", flush=True)
    return True
